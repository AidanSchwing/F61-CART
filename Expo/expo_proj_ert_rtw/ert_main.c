...
void rt_OneStep(void)
{
  ...
  __enable_irq();
  currentTime = (extmodeSimulationTime_T) expo_proj_M->Timing.taskTime0;
  expo_proj_step();
...
int main(int argc, char **argv)
{
  ...
  MX_TIM1_Init();
  rtmSetErrorStatus(expo_proj_M, 0);
  ...
  expo_proj_initialize();
  __disable_irq();
  __enable_irq();

  /* External Mode initialization */
  errorCode = extmodeInit(expo_proj_M->extModeInfo, &rtmGetTFinal(expo_proj_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(expo_proj_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(expo_proj_M);
  __enable_irq();
  __enable_irq();
  while (runModel) {
